...
int main(){
	...
	for(int k=t-1;~k;k--,now=!now){
		...
		for(int i=1;i<=n;i++)for(int _=head[i];_;_=e[_].next){
			if(is[_]>=k){
				if(e[_^1](!now)){
					e[_][now]=(dp[e[_][2]][!now]*cs[e[_][2]][!now]%mod-e[_^1][!now]+mod)%mod;
					e[_][now]=(e[_][now]*qk(cs[e[_][2]][!now]-1,mod-2)%mod+got(e[_][2],k+1))%mod;
				}else e[_][now]=(dp[e[_][2]][!now]+got(e[_][2],k+1))%mod;
				dp[i][now]=(dp[i][now]+e[_][now])%mod,cs[i][now]++,e[_](now)=true;
			}else if(goin[i]==1)dp[i][now]=(dp[i][now]+dp[i][!now]+got(i,k+1))%mod,cs[i][now]=1;
		}
